    ...
    def _parse_t_slice(self, t_slice):
        if isinstance(t_slice, slice):
            if t_slice.step is not None:
                ...
                raise IndexError(m)
            start = None if t_slice.start is None else int(t_slice.start * self.obj.fs)
            stop = None if t_slice.stop is None else int(t_slice.stop * self.obj.fs)
    ...
    def _parse_c_slice(self, c_slice):
        if isinstance(c_slice, slice):
            if c_slice.step is not None:
                ...
                raise IndexError(m)
            start = (
                None if c_slice.start is None else self.obj.chans.index(c_slice.start)
            )
            # Note that we add 1 to the label-based index becasue we're
            # duplicating Pandas loc behavior (where the end of a label
            # slice is included).
            stop = (
                None if c_slice.stop is None else self.obj.chans.index(c_slice.stop) + 1
            )
            return slice(start, stop)
...
class SignalBase:
    ...
    def _run_safety_checks(self):
        ...
        if self.chans:
            ...
            if not all(typesok):
                raise ValueError(
                    "Chans must be a list of strings:" + str(self.chans) + str(typesok)
                )
    ...
    def get_epoch_indices(
        ...
        bounds : 2D array (n_occurances x 2)
            ...
            time.
        """
        bounds = self.get_epoch_bounds(
            epoch, boundary_mode, fix_overlap, overlapping_epoch
        )
            ...
            for i, (lb, ub) in enumerate(indices):
                ...
                if np.all(m_data[0, lb:ub]) & np.all(m_data[0, lb]):
                    ...
                    if m_data[0, lb:ub].sum() != standard_mask.sum():
                        raise ValueError(
                            "For allow_incomplete=True, masks must all be the same size on each epoch"
                        )
    ...
    def _split_epochs(self, split_time):
        if self.epochs is None:
            ...
            if portion:
                warnings.warn(
                    "Epochs for {0} portion of signal: {1}"
                    "ended up empty after splitting by time.".format(portion, self.name)
                )
    ...
    def epoch_to_signal(
        ...
        attributes = self._get_attributes().copy()
    ...
    def extract_epoch(self, epoch, allow_empty=True, overlapping_epoch=None, mask=None):
        raise NotImplementedError
...
class RasterizedSignal(SignalBase):
    ...
    def __init__(
        ...
        if safety_checks and self.ntimes < self.nchans:
            m = (
                "Incorrect matrix dimensions?: (C, T) is {}. "
    ...
    def from_3darray(
        ...
        out = cls(
            fs, data, name, recording, chans, meta=meta, safety_checks=safety_cheks
        )
        times = np.array(
            [
                [t / fs, (t + n_times) / fs]
                for t in range(0, n_trials * n_times, n_times)
            ]
        )
    ...
    def as_file_streams(self, fmt=None):
        """
    ...
    def save(self, dirpath, fmt=None, prefix=""):
        """
        Save this signal to a CSV file + JSON sidecar. If desired,
        you may use optional parameter fmt (for example, fmt='%1.3e')
        to alter the precision of the floating point matrices.
    ...
    def extract_epoch(
        ...
        if epoch_indices.size == 0:
            if allow_empty:
                ...
                raise IndexError(
                    "No matching epochs to extract for: %s\n" "In signal: %s",
                    epoch,
                    self.name,
                )
        ...
        if data.dtype == bool:
            ...
            epoch_data = np.full(epoch_shape, np.nan, dtype=self.dtype)
        # print(epoch)
        ...
        for i, (lb, ub) in enumerate(epoch_indices):
            ...
            try:
                ...
                raise ValueError(
                    "Trying to extract invalid range from signal for epoch (out of bounds or negative duration?)."
                )
        return epoch_data
    ...
    def normalize(self, normalization="minmax", b=None, g=None, mask=None):
        ...
        m_normed, b, g = _normalize_data(
            m, normalization, d=b, g=g, sig=self, mask=mask
        )
    ...
    def normalized_by_mean(self):
        ...
        if not (hasattr(self, "channel_mean") and hasattr(self, "channel_std")):
        ...
        try:
            ...
            if prestimsilence.shape[-1] > 0:
                ...
                try:
                    ...
                    raise ValueError(
                        "Can't find prestim silence to use for PSTH calculation"
                    )
        except:
    ...
    def jackknife_by_epoch(
        ...
        if occurrences < njacks:
            raise ValueError(
                "Can't divide {0} occurrences into {1} jackknifes".format(
                    occurrences, njacks
                )
            )
    ...
    def jackknifes_by_epoch(self, njacks, epoch_name, tiled=True):
        ...
        while jack_idx < njacks:
            yield (
                self.jackknife_by_epoch(
                    njacks, jack_idx, epoch_name, tiled=tiled, invert=False
                ),
                self.jackknife_by_epoch(
                    njacks, jack_idx, epoch_name, tiled=tiled, invert=True
                ),
            )
            jack_idx += 1
    ...
    def jackknifes_by_time(self, njacks):
        ...
        while jack_idx < njacks:
            yield (
                self.jackknife_by_time(njacks, jack_idx, invert=False),
    ...
    def concatenate_channels(cls, signals):
        ...
        for signal in signals[1:]:
            ...
            if not base.ntimes == signal.ntimes:
                raise ValueError(
                    "Cannot concatenate signal with different number of samples"
                )
    ...
    def replace_epoch(self, epoch, epoch_data, preserve_nan=True, mask=None):
        ...
        if indices.size == 0:
            warnings.warn(
                "No occurrences of epoch were found: \n{}\n"
        ...
        for epoch, epoch_data in epoch_dict.items():
            ...
            if epoch_data.ndim == data.ndim:
                ...
                for lb, ub in indices:
                    ...
                    data[..., lb:ub] = data[..., lb:ub] + epoch_data[..., : (ub - lb)]
                ...
                for lb, ub in indices:
                    if ub > data.shape[-1]:
                        ub = data.shape[-1]
                    n = ub - lb
                    data[..., lb:ub] = data[..., lb:ub] + epoch_data[ii, ..., :n]
                    ii += 1
    ...
    def select_epoch(self, epoch):
        ...
        if np.all(np.isnan(new_data)):
            warnings.warn(
                "No matched occurrences for epoch: \n{}\n"
                "Returned signal will be only NaN.".format(epoch)
            )
    ...
    def select_epochs(self, list_of_epoch_names):
        ...
        if np.all(np.isnan(new_data)):
            warnings.warn(
                "No matched occurrences for epochs: \n{}\n"
                "Returned signal will be only NaN.".format(list_of_epoch_names)
            )
        return self._modified_copy(new_data)
    ...
    def trial_epochs_from_occurrences(self, occurrences=1):
        ...
        * Epoch indices behave similar to python list indices, so start is
        ...
        return pd.DataFrame({"start": starts, "end": ends, "name": "trial"})
    ...
    def to_epochs(self):
        ...
        return pd.DataFrame(
            {"start": starts / self.fs, "end": ends / self.fs, "name": self.name}
        )
    ...
    def rasterize(self, fs=None, sparse=False, dtype=None):
        """
...
class PointProcess(SignalBase):
    ...
    def rasterize(self, fs=None, sparse=False, dtype="float64"):
        """
        ...
        raster = np.zeros([unit_count, max_bin], dtype=dtype)
    ...
    def jackknife_by_epoch(
        ...
        """
        sig = self.rasterize()
        return sig.jackknife_by_epoch(
            njacks, jack_idx, epoch_name, tiled, invert, excise
        )
    ...
    def extract_epoch(
        ...
        if type(epoch) is str:
            epoch_bounds = self.get_epoch_bounds(
                epoch,
                boundary_mode=boundary_mode,
                fix_overlap=fix_overlap,
                mask=mask,
        ...
        for j, c in enumerate(self._data.keys()):
            ...
            epoch_data[c] = np.concatenate(
                (np.expand_dims(t, 1), np.expand_dims(d, 1)), axis=1
            )
    ...
    def __init__(
        ...
        for k, v in data.items():
            ...
            if chancount and this_chancount != chancount:
                raise ValueError(
                    "channel count does not match across tiled signal dictionary"
                )
...
def load_signal_from_streams(data_stream, json_stream, epoch_stream=None):
    ...
    if "RasterizedSignal" in signal_type:
        ...
        if not data:
            warnings.warn(
                "Tried to load data stream {0} but data object"
                "ended up empty. Potential bug upstream?".format(data_stream)
            )
...
def _split_epochs(epochs, split_time):
    ...
    if portion:
        warnings.warn(
            "Epochs for {0} portion of signal"
            "ended up empty after splitting by time.".format(portion)
        )
