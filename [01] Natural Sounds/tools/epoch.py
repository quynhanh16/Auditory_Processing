...
def remove_overlap(a):
    """
    Remove overlapping occurrences by taking the first occurrence
    """
    a = a.copy()
    a.sort(axis=0)
    i = 0
...
def epoch_union(a, b):
    ...
    union : 2D array of (O x 2)
        ...
        is the number of occurrences of the union of a and b. Note that O <= M +
        N.
...
def epoch_difference(a, b):
    ...
    difference : 2D array of (O x 2)
        The first column is the start time and second column is the end time. O
        is the number of occurrences of the difference of a and b.
...
def find_common_epochs(epochs, epoch_name, d=12):
    ...
    occurrences of `epoch_name`. An epoch is considered "common" to all
    occurrences if the name matches and the start and end times, relative to the
    ...
    common_epochs : dataframe with 'name', 'start', 'end'
        Epochs common to all occurrences of `epoch_name`. The start and end
    ...
    for lb, ub in matches[["start", "end"]].values:
        ...
        epoch_subset["end"] -= lb
        epoch_subset = set(
            (n, round(s, d), round(e, d))
            for (n, s, e) in epoch_subset[["name", "start", "end"]].values
        )
    ...
    new_epochs = pd.DataFrame(list(common_epochs), columns=["name", "start", "end"])
...
def append_epoch(epochs, epoch_name, epoch):
    ...
    _df = pd.DataFrame(
        {"name": epoch_name, "start": epoch[0], "end": epoch[1]}, index=[0]
    )
