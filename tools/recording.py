...
class Recording:
    ...
    @staticmethod
    def load_from_arrays(arrays, rec_name, fs, sig_names=None, signal_kwargs={}):
        """
        DEPRECATED???
    ...
    def from_nwb(cls, nwb_file, nwb_format):
        ...
        assert nwb_format in [
            "neuropixel"
        ], f'"{nwb_format}" not a supported NWB file format.'
        ...
        if nwb_format == "neuropixel":
            ...
            try:
                ...
                from allensdk.brain_observatory.ecephys import (
                    nwb,
                )  # needed for ecephys format compat
            except ImportError:
            ...
            with NWBHDF5IO(str(nwb_filepath), "r") as nwb_io:
                ...
                # needs to be a dict
                units_meta = pd.DataFrame(units_data, index=units.id[:]).to_dict(
                    "index"
                )
                ...
                epoch_data = {
                    ...
                    if col.name
                    not in ["tags", "timeseries", "tags_index", "timeseries_index"]
                }
    ...
    def save(self, uri="", fmt=None, uncompressed=False):
        """
        Saves this recording to a URI as a compressed .tgz file.
        :param uri: output location, if path only, filename generated automatically
        based on self.name
        :param fmt: Optional argument to specify how floating point numbers are saved in
        csv files (default is '%.18e')
        :param uncompressed: Optional argument 'uncompressed' may be used to force the save
        to occur as a directory full of uncompressed files, but this only
        works for URIs that point to the local filesystem.
        :return: URI of what was saved, or None if there was a problem.
        ...
        if local_uri(uri):
            ...
            if targz_uri(uri):
                return self.save_targz(uri, fmt=fmt)
            elif uncompressed:
    ...
    def split_using_epoch_occurrence_counts(
        self,
        epoch_regex=None,
        keepfrac=1,
        selection=None,
        ...
        **context,
        ...
        if selection is not None:
            ...
            if selection == "mono":
                if len(mono_epochs) > len(bin_epochs):
                    ...
                    log.info(
                        "Mono stim only in fit set. Slicing subset to match len(bin_epochs)"
                    )
                else:
                ...
                lo_rep_epochs = (
                    mono_epochs[slice(0, l * 2, 2)]
                    + bin_epochs[slice(0, (minlen - l) * 2, 2)]
                )
            else:
        ...
        if keepfrac < 1:
            log.info(
                f"keepfrac={keepfrac}, keeping {keep_count}/{lo_count} low-rep epochs"
            )
    ...
    def jackknife_mask_by_epoch(
        ...
        epochs = self.get_epoch_indices(
            epoch_name, allow_partial_epochs=allow_partial_epochs
        )
    ...
    def jackknife_masks_by_epoch(
        self, njacks, epoch_name, tiled=True, invert=False, allow_partial_epochs=False
    ):
        signal_views = []
        for jack_idx in range(njacks):
            trec = self.jackknife_mask_by_epoch(
                njacks,
                jack_idx,
                epoch_name,
                tiled,
    ...
    def jackknife_mask_by_time(self, njacks, jack_idx, tiled=True, invert=False):
        """
    ...
    def jackknife_by_epoch(
        ...
        for sn in self.signals.keys():
            if not only_signals or sn in set(only_signals):
                s = self.signals[sn]
                log.debug(
                    "JK: {0} {1}/{2} {3}".format(s.name, jack_idx, njacks, epoch_name)
                )
                new_sigs[sn] = s.jackknife_by_epoch(
                    njacks, jack_idx, epoch_name=epoch_name, invert=invert, tiled=tiled
                )
        return Recording(signals=new_sigs)

    def jackknife_by_time(
        self, nsplits, split_idx, only_signals=None, invert=False, excise=False
    ):
        """
        ...
        for sn in self.signals.keys():
            if not only_signals or sn in set(only_signals):
                s = self.signals[sn]
                new_sigs[sn] = s.jackknife_by_time(
                    nsplits, split_idx, invert=invert, excise=excise
                )
        return Recording(signals=new_sigs)
...
DEMO_NAMES = [
    "TAR010c-18-1.pkl",
    "TAR010c-NAT-stim.csv.gz",
    "TAR010c-NAT-resp.csv.gz",
    "TAR010c-NAT-epochs.csv",
    "TAR010c.NAT.fs100.ch18.tgz",
    "eno052d-a1.tgz",
    "BRT026c-02-1.tgz",
    "resp2.tgz",
    "TAR010c-06-1.tgz",
        ...
        for uri in uris:
            ...
            if os.path.isfile(local):
                ...
                try:
                    ...
                    log.warn(
                        "Couldn't write in directory: \n{}\n"
                        "due to permission issues. Make sure the "
                        "parent directory grants write permission.".format(directory)
                    )
                    log.exception(e)
    return directory
